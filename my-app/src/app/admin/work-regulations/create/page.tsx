...
import { useCreateWorkingRulesMutation } from '@/services/WorkingRulesService'
...
function CreateConfigurationPage() {
    ...
    const [createDepartment, { isSuccess, isLoading, isError }] = useCreateWorkingRulesMutation()
    ...
    useEffect(() => {
        if (isSuccess === true) {
            toast(t('Tạo quy định thàng công'), 'success')
        }
        if (isError === true) {
            toast(t('Tạo quy định thất bại'), 'error')
        }
    }, [isSuccess, isError])
