...
import { useGetAllRolesQuery, useChangeStatusMutation } from '@/services/AspNetRoleService'
...
function DepartmentTable() {
    ...
    const { data: responseData, isFetching, refetch, isLoading: LoadingDepartment } = useGetAllRolesQuery(filter)
    const [deleteDepartment, { isSuccess: isSuccessDelete }] = useChangeStatusMutation()

    const [isSuccess] = useState(false)

    const departmentdata = responseData?.Data.Records as IAspNetRoleGetAll[]
                            ...
                            <TableRow sx={{ backgroundColor: 'var(--header-color-table)' }}>
                                ...
                                <TableCell sx={{ borderColor: 'var(--border-color)' }}>
                                    ...
                                    >
                                        ...
                                        >
                                            {t('Tên chức vụ')} {/* Cập nhật khóa dịch này */}
                                        </Typography>
                                    </TableSortLabel>
                                </TableCell>
                                ...
                                <TableCell sx={{ borderColor: 'var(--border-color)' }}>
                                    <TableSortLabel
                                        active={orderBy === 'LevelRole'}
                                        direction={orderBy === 'LevelRole' ? order : 'asc'}
                                        onClick={() => handleSort('LevelRole')}
                                        sx={{
                                            '& .MuiTableSortLabel-icon': {
                                                color: 'var(--text-color) !important'
                                            }
                            ...
                            {departmentdata?.map(row => (
                                <TableRow key={row.Id} selected={isSelected(row.Id)}>
                                    ...
                                    <TableCell sx={{ borderColor: 'var(--border-color)' }}>
                                        ...
                                        >
                                            {row.IsAdmin === true ? t('Cho phép') : t('Không cho phép')}
                                        </Typography>
                                    </TableCell>
                                        ...
                                        >
                                            <Tooltip title={t('COMMON.BUTTON.EDIT')}>
                                                <Box
                                                    ...
                                                    onClick={() => router.push(`/admin/role/update?id=${row.Id}`)}
                                                >
                                                    <Pencil />
                                                </Box>
                                            </Tooltip>
