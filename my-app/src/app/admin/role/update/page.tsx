...
import { useUpdateRolesMutation, useGetByIdRolesQuery } from '@/services/AspNetRoleService'
...
function CreateConfigurationPage() {
    ...
    const { data: response, isFetching: isFetchingGetById, refetch: fetch } = useGetByIdRolesQuery(id)

    const data = response?.Data
    useEffect(() => {
        if (!isFetchingGetById && data) {
            setName(data.Name || '')
            setLevelRole(data.LevelRole || '')
            setIsAdmin(data.IsAdmin === true ? 'co' : 'ko')
        }
    }, [data, isFetchingGetById])

    const [update, { isSuccess, isError, reset, isLoading }] = useUpdateRolesMutation()
    ...
    useEffect(() => {
        if (isSuccess === true) {
            toast(t('Cập nhật chức vụ thàng công'), 'success')
            fetch()
            reset()
        }
        if (isError === true) {
            toast(t('Cập nhật chức vụ thất bại'), 'error')
            fetch()
            reset()
        }
    }, [isSuccess, isError])
